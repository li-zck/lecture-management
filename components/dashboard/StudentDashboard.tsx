...
export function StudentDashboard() {
    ...
    async function fetchData() {
        try {
            setLoading(true);
            const [profileData, coursesData, gradesData, scheduleData] =
                await Promise.all([
    ...
    const handleWithdraw = async (enrollmentId: string) => {
        try {
            if (!confirm("Are you sure you want to withdraw from this course? Query cannot be undone.")) return;
        ...
        <div className="container mx-auto p-6 max-w-6xl">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-md font-medium transition ${activeTab === "enroll"
                        ? "bg-primary text-primary-foreground"
                        : "bg-muted hover:bg-muted/80"
                        }`}
                >
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-md font-medium transition ${activeTab === "grades"
                        ? "bg-primary text-primary-foreground"
                        : "bg-muted hover:bg-muted/80"
                        }`}
                >
                                ...
                                <CardContent>
                                    ...
                                    <div className="flex gap-2">
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            onClick={() => handleViewDocuments(enrollment.enrollmentId)}
                                        >
                                            View Documents
                                        </Button>
                                        ...
                                        {enrollment.grades.finalGrade === null &&
                                            enrollment.grades.gradeType1 === null && (
                                                <Button
                                                    variant="destructive"
                                                    size="sm"
                                                    onClick={() => handleWithdraw(enrollment.enrollmentId)}
                                                >
                                                    Withdraw
                                                </Button>
                                            )}
                                        ...
                                        <div className="mt-4 p-4 bg-muted rounded-md animate-in slide-in-from-top-2">
                                            ...
                                            {loadingDocs ? (
                                                ...
                                                <ul className="space-y-2">
                                                    {documents.map((doc) => (
                                                        <li key={doc.id} className="flex justify-between items-center p-2 bg-background rounded border">
                                        ...
                                        >
                                            <div className="mb-4 md:mb-0">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <h3 className="font-bold text-lg">{course.course.name}</h3>
                                                    <Badge variant="outline">{course.course.credits} Credits</Badge>
                                                </div>
                                                <p className="text-sm text-muted-foreground mb-1">
                                                    {course.lecturer?.fullName || "TBA"} • {course.semester}
                                                </p>
                                                <p className="text-sm">
                                                    {course.schedule.dayOfWeek !== null ? DAY_NAMES[course.schedule.dayOfWeek] : "TBA"} • {formatTime(course.schedule.startTime)} - {formatTime(course.schedule.endTime)}
                                                </p>
                        ...
                        {grades.length === 0 ? (
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    ...
                                    <tbody>
                                        {grades.map((grade) => (
                                            <tr key={grade.courseName + grade.semester} className="border-b">
